...
FARM_OPERATIONS_START_DATE = datetime.date(1900, 1, 1)
...
# Initial Load VS Incremental MERGE
if not spark.catalog.tableExists(table_name):
    window_fac = Window.partitionBy("facility_id").orderBy("effective_date")

    dim_fac_initial = (
        dim_fac_stg
        .withColumn("row_num", F.row_number().over(window_fac))
        # 🔑 FIX: Guarantee the earliest historical baseline starts at 1900-01-01
        .withColumn("effective_date", F.when(F.col("row_num") == 1, F.lit(FARM_OPERATIONS_START_DATE).cast("date")).otherwise(F.col("effective_date")))
        ...
        .select(
            F.abs(F.xxhash64(F.concat_ws("||", F.col("facility_id"), F.col("effective_date").cast("string")))).alias("facility_key"),
            F.col("facility_id"), 
            F.col("facility_name"), 
            F.col("region"), 
            F.col("short_region"),
    ...
    dim_facility_final.write.format("delta").mode("overwrite").option("mergeSchema", "true").saveAsTable(table_name)
    ...
    df_new_fac_keys = dim_fac_stg.alias("src").join(target_fac.alias("tgt"), "facility_id", "left_anti")\
                        .select(
                            F.abs(F.xxhash64(F.concat_ws("||", F.col("src.facility_id"), F.lit(PIPELINE_RUN_DATE).cast("string")))).alias("facility_key"),
    ...
    delta_fac.alias("target").merge(
        ...
        set={
            "expiration_date": F.date_sub(F.lit(PIPELINE_RUN_DATE).cast("date"), 1),
    ...
    if not df_fac_to_append.isEmpty():
        df_fac_to_append.write.format("delta").mode("append").option("mergeSchema", "true").saveAsTable(table_name)
...
# Explicit StructType Schema Definition (13 Columns including facility_id)
...
df_risk_eq = (
    spark.table("silver.equipment_risk_cleaned")
    .filter(
    ...
    .withColumn("installation_date", F.lit(FARM_OPERATIONS_START_DATE).cast("date"))
)

df_eq_union = (
    df_master_eq.unionByName(df_risk_eq)
...
if not spark.catalog.tableExists(table_name):
    ...
    dim_eq_stg_init = (
        dim_eq_stg
        .withColumn("row_num", F.row_number().over(window_eq))
        .withColumn("effective_date", F.when(F.col("row_num") == 1, F.lit(FARM_OPERATIONS_START_DATE).cast("date")).otherwise(F.col("effective_date")))
        ...
        .drop("row_num", "lead_eff_date")
    )

    dim_eq_initial = (
        dim_eq_stg_init.alias("stg")
        .join(
            dim_fac_bcast.alias("fac"),
            (F.col("stg.facility_id") == F.col("fac.facility_id")) &
        ...
        .select(
            F.abs(F.xxhash64(F.concat_ws("||", F.upper(F.trim(F.col("stg.equipment_id"))), F.col("stg.effective_date").cast("string")))).alias("equipment_key"),
            F.coalesce(F.col("fac.facility_key"), F.lit(-1)).alias("facility_key"),
    ...
    dim_equipment_final.write.format("delta").mode("overwrite").option("mergeSchema", "true").saveAsTable(table_name)
    ...
    df_new_eq_keys = (
        dim_eq_stg_resolved.alias("src")
        .join(target_eq.alias("tgt"), ["equipment_id"], "left_anti")
        .select(
            F.abs(F.xxhash64(F.concat_ws("||", F.upper(F.trim(F.col("src.equipment_id"))), F.col("src.effective_date").cast("string")))).alias("equipment_key"),
...
# Crop Master Dimension (gold.dim_crop - 18 Cultivars + 1 Required Unknown Dimension Member)
...
# 1. Complete Official Cultivar Reference Matrix (All 18 Commercial Philippine Hydroponic Crops)
CULTIVAR_REFERENCE = [
    ("arugula", 20.0, 60.0, 120.0, 30, 580.0, 377.0),
    ("baby_greens", 21.0, 65.0, 100.0, 25, 650.0, 422.5),
    ("basil", 24.0, 65.0, 140.0, 35, 750.0, 487.5),
    ("batavia_lettuce", 21.5, 65.0, 180.0, 40, 420.0, 273.0),
    ("butterhead_lettuce", 22.0, 65.0, 200.0, 42, 450.0, 292.5),
    ("cherry_tomatoes", 24.0, 70.0, 350.0, 60, 680.0, 442.0),
    ("cilantro", 20.0, 60.0, 110.0, 30, 520.0, 338.0),
    ("kale", 20.0, 60.0, 220.0, 45, 580.0, 377.0),
    ("mint", 22.0, 65.0, 130.0, 32, 700.0, 455.0),
    ("oakleaf_lettuce", 21.0, 65.0, 170.0, 38, 430.0, 279.5),
    ("parsley", 20.0, 60.0, 120.0, 35, 600.0, 390.0),
    ("pea_shoots", 20.0, 60.0, 90.0, 18, 950.0, 617.5),
    ("radish_microgreens", 21.0, 60.0, 80.0, 14, 950.0, 617.5),
    ("romaine", 21.0, 65.0, 250.0, 45, 420.0, 273.0),
    ("spinach", 19.0, 65.0, 160.0, 35, 480.0, 312.0),
    ("strawberry", 19.0, 70.0, 280.0, 65, 1400.0, 910.0),
    ("sweet_basil", 24.0, 65.0, 150.0, 35, 780.0, 507.0),
    ("watercress", 19.5, 70.0, 130.0, 28, 620.0, 403.0)
]

schema_crop = StructType([
    StructField("crop_type", StringType(), False),
    StructField("optimal_temperature_celsius", DoubleType(), True),
    StructField("optimal_humidity_percent", DoubleType(), True),
    StructField("target_biomass_g", DoubleType(), True),
    StructField("harvest_cycle_days", IntegerType(), True),
    StructField("unit_price_grade_a_php", DoubleType(), True),
    StructField("unit_price_grade_b_php", DoubleType(), True)
])

df_cultivars = spark.createDataFrame(CULTIVAR_REFERENCE, schema=schema_crop)

dim_crop_body = df_cultivars.select(
    ...
    F.current_timestamp().alias("created_timestamp"),
    F.lit(PIPELINE_RUN_DATE).alias("pipeline_run_date")
).drop_duplicates(["crop_key"])

# 2. Append Required Unknown (-1) Dimension Member
unknown_crop = spark.createDataFrame([(
    -1, "Unknown Crop", 22.0, 65.0, 150.0, 35, 480.0, 312.0, datetime.datetime.now(), PIPELINE_RUN_DATE
)], schema=dim_crop_body.schema)

dim_crop_final = unknown_crop.unionByName(dim_crop_body)
...
validation_status = "PASSED" if (total_rows == 19 and unknown_count == 1) else "FAILED"

print(f"⭐ TABLE: {table_name} - Total Rows: {total_rows} | Unknown (-1): {unknown_count} | Status: {validation_status}")
...
# Read Source Data and Filter Anomaly Keys
df_env_raw = (
    spark.table("silver.environmental_metrics")
    .filter(
...
env_cols = df_env_raw.columns
temp_col   = "air_temperature_c" if "air_temperature_c" in env_cols else "air_temperature"
humid_col  = "humidity_pct" if "humidity_pct" in env_cols else "relative_humidity"
co2_col    = "co2_ppm" if "co2_ppm" in env_cols else "co2_level"
vpd_col    = "vpd_kpa" if "vpd_kpa" in env_cols else "vpd"
drift_col  = "temp_drift_c" if "temp_drift_c" in env_cols else "temperature_drift"
stab_col   = "composite_stability_score" if "composite_stability_score" in env_cols else "stability_score"
ph_col     = "water_ph" if "water_ph" in env_cols else "water_ph"
ec_col     = "electrical_conductivity_ms_cm" if "electrical_conductivity_ms_cm" in env_cols else "water_ec"
sample_col = "snapshot_id" if "snapshot_id" in env_cols else "event_id"
...
df_env_agg = (
    ...
    .agg(
        F.round(F.avg(temp_col), 2).alias("avg_ambient_temp_celsius"),
        F.round(F.min(temp_col), 2).alias("min_ambient_temp_celsius"),
        F.round(F.max(temp_col), 2).alias("max_ambient_temp_celsius"),
        F.round(F.avg(humid_col), 2).alias("avg_humidity_pct"),
        F.round(F.avg(co2_col), 2).alias("avg_co2_ppm"),
        F.round(F.avg(vpd_col), 2).alias("avg_vpd_kpa"),
        F.round(F.avg(drift_col), 2).alias("avg_temp_drift_celsius"),
        F.round(F.avg(stab_col), 2).alias("avg_stability_score"),
        F.round(F.avg(ph_col), 2).alias("avg_water_ph"),
        F.round(F.avg(ec_col), 2).alias("avg_ec_ms_cm"),
        F.count(sample_col).alias("telemetry_sample_count")
    )
)
...
dim_zone_bcast = F.broadcast(
    spark.table("gold.dim_zone")
    .select("zone_key", "facility_id", "zone_id", "effective_date", "expiration_date")
    .cache()
)
...
eq_cols = df_eq_raw.columns
health_col  = "equipment_health_status" if "equipment_health_status" in eq_cols else ("avg_health_score" if "avg_health_score" in eq_cols else "health")
fail_col    = "failure_probability" if "failure_probability" in eq_cols else "max_failure_probability"
power_col   = "power_consumption_kw" if "power_consumption_kw" in eq_cols else "power_draw_kw"
vib_col     = "vibration_vps" if "vibration_vps" in eq_cols else "vibration"
temp_col    = "operating_temp_c" if "operating_temp_c" in eq_cols else ("operating_temperature_c" if "operating_temperature_c" in eq_cols else "temp_c")
load_col    = "current_load_percent" if "current_load_percent" in eq_cols else ("current_load" if "current_load" in eq_cols else "load_percent")
...
df_eq_agg = (
    ...
    .agg(
        ...
        F.round(F.avg(load_col), 1).alias("avg_load_percent"),
...
df_crop_raw = (
    spark.table("silver.crop_biological_cleaned")
    .filter(
        (F.col("facility_id").rlike("^FAC-[0-9]{3}$")) &
        (F.col("zone_id").rlike("^ZONE-[0-9]{3}$")) &
        (F.col("zone_id") != "ZONE-000") &
        (F.col("timestamp").isNotNull()) &
        (F.year("timestamp") >= 2025)
    )
)
...
df_yield_agg = (
    ...
    .withColumn(
        ...
        F.round(F.col("grade_a_harvest_kg") * 480.0 + F.col("grade_b_harvest_kg") * 312.0, 2)
    )
)
...
dim_crop_bcast = F.broadcast(
    ...
    .select("crop_key", F.upper(F.trim(F.col("crop_type"))).alias("crop_id"))
    .cache()
)
...
fact_crop_stg = (
    ...
    .filter(F.col("date_key") >= 20250101)
...
print(f"⭐ TABLE: {table_name} - Saved {df_fact_crop.count():,} certified rows.")
...
print(f"⭐ TABLE: {table_name} - Saved {df_fact_irr.count():,} certified rows.")
...
# 1. Read Source Data and Filter to Valid Operations (2025+ Certified Data Only)
df_maint_raw = (
    spark.table("silver.maintenance_sla_cleaned")
    .filter(
        (F.col("facility_id").rlike("^FAC-[0-9]{3}$")) &
        (F.col("zone_id").rlike("^ZONE-[0-9]{3}$")) &
        (F.col("zone_id") != "ZONE-000") &
...
# 2. Daily Aggregation per Business Grain
df_maint_agg = (
    df_maint_raw
...
# 3. Cached Dimension Broadcast Lookups
dim_fac_bcast = F.broadcast(
    spark.table("gold.dim_facility")
    .select("facility_key", "facility_id", "effective_date", "expiration_date")
    .cache()
...
# 4. Strict Inner Joining to Eliminate Orphaned / Blank Keys
fact_maint_stg = (
    ...
    .filter(F.col("date_key") >= 20250101)
    .filter((F.col("facility_key") != -1) & (F.col("zone_key") != -1) & (F.col("equipment_key") != -1) & (F.col("technician_key") != -1))
...
elapsed_time = round(time.time() - cell_start_time, 2)
validation_status = "PASSED" if (unknown_fac_count == 0 and unknown_zone_count == 0 and unknown_eq_count == 0 and unknown_tech_count == 0) else "FAILED"

print("==============================================================================")
print(f"⭐ TABLE: {table_name} (DYNAMIC PARTITION OVERWRITE)")
print("==============================================================================")
print(f"Source Rows Read:         {source_row_count:,}")
print(f"Total Certified Records:  {total_rows:,}")
print(f"Distinct Business Grain:  {distinct_grain:,}")
...
df_dl_agg = df_dl_raw \
    ...
    .withColumn("governance_exception_reason", gov_reason_canonical) \
...
gold_tables = [
    ...
    ("gold.fact_environmental_daily", "Fact (Daily Agg)", ["facility_key", "zone_key"]),
    ("gold.fact_equipment_telemetry", "Fact (Daily Agg)", ["facility_key", "equipment_key", "zone_key"]),
    ("gold.fact_crop_yield", "Fact (Daily Agg)", ["facility_key", "zone_key", "crop_key"]),
    ("gold.fact_irrigation_daily", "Fact (Daily Agg)", ["facility_key", "zone_key"]),
    ("gold.fact_lighting_dli_daily", "Fact (Daily Agg)", ["facility_key", "zone_key"]),
    ("gold.fact_maintenance_sla", "Fact (Daily Agg)", ["facility_key", "zone_key", "equipment_key", "technician_key"]),
    ("gold.fact_dead_letter_governance", "Fact (Governance)", [])
]
...
for table_name, table_type, key_columns in gold_tables:
    if spark.catalog.tableExists(table_name):
        ...
        # Rigorous Unknown Key (-1) Verification across ALL keys
        total_unknown_in_table = 0
        table_passed = True
        
        if table_name.startswith("gold.dim_"):
            for k in key_columns:
                if k in df_tbl.columns:
                    u_cnt = df_tbl.filter(F.col(k) == -1).count()
                    total_unknown_in_table += u_cnt
                    if u_cnt != 1:
                        table_passed = False
        elif table_name == "gold.fact_dead_letter_governance":
            table_passed = True
        else:
            for k in key_columns:
                if k in df_tbl.columns:
        ...
        print(f"{table_name:<34} | {table_type:<18} | {rowCount:<10,} | {total_unknown_in_table:<12} | {status:<8}")
        audit_results.append((table_name, table_type, rowCount, total_unknown_in_table, status))
    else:
        status = "MISSING"
        overall_passed = False
...
elapsed_time = round(time.time() - cell_start_time, 2)
overall_status = "ALL TABLES PASSED & VERIFIED" if overall_passed else "ATTENTION REQUIRED (FAILED CHECKS)"
print("=" * 92)
