...
# Enterprise Calendar Dimension (gold.dim_date)
...
# Automatic extension check & calendar generation
needs_rebuild = True
if spark.catalog.tableExists(table_name):
    max_date_row = spark.table(table_name).select(F.max("full_date")).collect()[0][0]
    if max_date_row and max_date_row >= target_end_date:
        needs_rebuild = False

# One-time creation check & calendar generation
if needs_rebuild:
    ...
    unknown_date = spark.createDataFrame([(
        -1, datetime.date(1900, 1, 1), 1900, 1, 1, "Unknown", 1, 1, "Unknown", 1, 1, False, 1900, 1, 1, False, False, False
    ...
    action_type = "INITIALIZED & CREATED"

else:
    action_type = "REUSED (EXISTS)"
...
print(f"Null Keys Count:      {null_keys}")
...
# Prepare staging Dataframe with inline  attribute hash computation using native Pyspark APIs
...
if not spark.catalog.tableExists(table_name):
    ...
    unknown_fac = spark.createDataFrame([(
        -1, "UNKNOWN", "Unknown Facility", "N/A", "N/A", 0.0, 0.0, 0.0, "N/A", "N/A", "N/A", 0, "N/A", 0,
    ...
    # Identify changed business keys (attr_hash differ from active target)
    df_changed_fac_keys = dim_fac_stg.alias("src")\
                        .join(target_fac.alias("tgt"), "facility_id")\
    ...
    # Pass 1: Expire active records in Target Delta table where attributes changed
    ...
    # Pass 2: Append only brand-new keys and newly reated active replacement versions
    df_fac_to_append = df_new_fac_keys.unionByName(df_changed_fac_keys).drop_duplicates(["facility_id", "effective_date"])

    # Lightweight Pyspark existence check avoids triggering full table scans
...
# Conformed Zone Dimension (gold.dim_zone - SCD Type 2)
...
# Explicit StructureType Schema Definition
...
# Read Source Data and Compute Attribute Hash
# Combine distinct zone IDs from environmental and equipment streams
...
# Rich category and section mapping per zone
zone_name_calc = (
...
# Check if table needs a baseline rebuild due to uncleaned ZONE-UNKNOWN records
...
# Read source data & compute attribute hash (Union Master + Telemetry Assets)
...
# Select equipment business attributes and compute change detection hash using EQUIPMENT_TRACKED_COLUMNS
...
# Two-pass SCD Type 2 Processing (Initial Baseline Load)
...
if not spark.catalog.tableExists(table_name):
    ...
    dim_eq_initial = dim_eq_stg_init.alias("stg") \
        ...
        .select(
            F.abs(F.xxhash64(F.concat_ws("||", F.upper(F.trim(F.col("stg.equipment_id"))), F.lit(FARM_OPERATIONS_START_DATE).cast("string")))).alias("equipment_key"),
            F.coalesce(F.col("fac.facility_key"), F.lit(-1)).alias("facility_key"),
    ...
    dim_equipment_final = unknown_eq.unionByName(dim_eq_initial)
    ...
    df_new_eq_keys = dim_eq_stg_resolved.alias("src") \
        ...
        .select(
            F.abs(F.xxhash64(F.concat_ws("||", F.upper(F.trim(F.col("src.equipment_id"))), F.lit(PIPELINE_RUN_DATE).cast("string")))).alias("equipment_key"),
    ...
    df_eq_to_append = df_new_eq_keys.unionByName(df_changed_eq_keys).drop_duplicates(["equipment_id", "effective_date"])
    
    if not df_eq_to_append.isEmpty():
        df_eq_to_append.write.format("delta") \
            .mode("append") \
            .option("mergeSchema", "true") \
            .saveAsTable(table_name)
...
distinct_b_keys = df_dim_eq.select("equipment_id").distinct().count()
duplicate_active_keys = df_dim_eq.filter("is_current = true AND equipment_key != -1").groupBy("equipment_id").count().filter("count > 1").count()
null_b_keys = df_dim_eq.filter(F.col("equipment_id").isNull()).count()
unknown_count = df_dim_eq.filter(F.col("equipment_key") == -1).count()
current_active_rows = df_dim_eq.filter(F.col("is_current") == True).count()
historical_inactive_rows = df_dim_eq.filter(F.col("is_current") == False).count()
...
# Crop Master Dimension (gold.dim_crop - SCD Type 1)
...
# Select distinct crop types and generate SCD Type 1 surrogate keys
...
unknown_crop = spark.createDataFrame([(
    -1, "Unknown Crop", 22.0, 65.0, 150.0, 35, datetime.datetime.now(), PIPELINE_RUN_DATE
...
# Crop Yield Fact (gold.fact_crop_yield)
...
# Daily aggregation per facility, zone and crop cultivar
...
df_yield_agg = df_crop_raw \
    ...
    .agg(
        ...
        F.round(F.max("biomass_g") / 1000.0, 2).alias("target_yield_kg"),
        F.round(F.avg(F.when(F.col("crop_health_score") >= 80, F.col("biomass_g")).otherwise(F.col("biomass_g") * 0.6)) / 1000.0, 2).alias("total_harvest_kg"),
        F.round(F.avg(F.col("biomass_g") * 0.80) / 1000.0, 2).alias("grade_a_harvest_kg"),
        F.round(F.avg(F.col("biomass_g") * 0.15) / 1000.0, 2).alias("grade_b_harvest_kg"),
        F.round(F.avg(F.col("biomass_g") * 0.05) / 1000.0, 2).alias("spoilage_waste_kg"),
    ...
    .withColumn(
        ...
        F.round(F.col("grade_a_harvest_kg") * 280.0 + F.col("grade_b_harvest_kg") * 180.0, 2)
    )
...
# Dead-Letter Governance Fact (gold.fact_dead_letter_governance)
...
# Read Source Data and Aggregated Daily Dead Letter Defect Metrics
...
# Daily aggregation per date, target stream, and exception reason
df_dl_agg = df_dl_raw \
    .withColumn("event_date", F.to_date(F.col(time_col))) \
    .withColumn("date_key", F.date_format(F.col(time_col), "yyyyMMdd").cast("int")) \
    ...
    .agg(
        ...
        F.sum(F.when(F.col(reason_col).contains("DEPRECATED"), 1).otherwise(0)).alias("deprecated_schema_defect_count"),
        F.sum(F.when(F.col(reason_col).contains("TYPE") | F.col(reason_col).contains("FORMAT"), 1).otherwise(0)).alias("formatting_defect_count")
    )

# Resolve Data Surrogate Key Staging
